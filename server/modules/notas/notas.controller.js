...
const getNotas = async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      `SELECT id, scope, referencia_id, user_id, contenido, created_at, titulo, tipo, actividad_id, usuario_id, fecha_creacion, fecha_actualizacion, updated_at
       FROM notas ${whereClause} ORDER BY COALESCE(fecha_creacion, created_at) DESC`,
      params
    );
    const notes = result.rows.map((row) => ({
      ...
      actividadId: row.actividad_id ?? row.referencia_id ?? null,
      scope: row.scope,
      referenciaId: row.referencia_id,
      userId: row.usuario_id ?? row.user_id,
      contenido: row.contenido,
      createdAt: row.fecha_creacion ?? row.created_at,
      updatedAt: row.fecha_actualizacion ?? row.updated_at ?? null
    }));
...
const createNota = async (req, res, next) => {
  try {
    ...
    const query = `
      INSERT INTO notas
        (titulo, contenido, tipo, actividad_id, scope, referencia_id, user_id, usuario_id, created_at, updated_at, fecha_creacion, fecha_actualizacion)
      VALUES
        ($1, $2, $3, $4, $5, $6, $7, $7, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      RETURNING id, titulo, contenido, tipo, actividad_id, scope, referencia_id, usuario_id, user_id, fecha_creacion, fecha_actualizacion, created_at
    `;
    const params = [titulo, contenido, tipo, actividadId, scope, referenciaId, req.user.id];
    ...
    return success(res, 'Nota creada', {
      ...
      contenido: row.contenido,
      createdAt: row.fecha_creacion ?? row.created_at,
      updatedAt: row.fecha_actualizacion ?? null
    });
  } catch (error) {
    next(error);
  }
